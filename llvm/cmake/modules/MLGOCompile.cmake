...
# "ClassName,cli-flag,path/to/model.mlir,type". Entries whose type matches
# ${target_type} are lowered with mlir-opt and translated with
# mlir-translate to a C++ header defining ClassName, placed in
...
function(mlgo_compile_models models target_type include_subdir def_file
         umbrella_header)
  if (LLVM_NATIVE_TOOL_DIR AND EXISTS "${LLVM_NATIVE_TOOL_DIR}/mlir-opt${LLVM_HOST_EXECUTABLE_SUFFIX}")
    set(mlir_opt_exe "${LLVM_NATIVE_TOOL_DIR}/mlir-opt${LLVM_HOST_EXECUTABLE_SUFFIX}")
    set(mlir_opt_target "")
  elseif (TARGET "host_mlir-opt")
    get_native_tool_path(mlir-opt mlir_opt_exe)
    set(mlir_opt_target "host_mlir-opt")
  elseif (LLVM_USE_HOST_TOOLS)
    get_native_tool_path(mlir-opt mlir_opt_exe)
    set(mlir_opt_target "host_mlir-opt")
    build_native_tool(mlir-opt mlir_opt_exe)
    add_custom_target(host_mlir-opt DEPENDS ${mlir_opt_exe})
    get_subproject_title(subproject_title)
    set_target_properties(host_mlir-opt PROPERTIES FOLDER "${subproject_title}/Native")
  elseif (TARGET "mlir-opt")
    set(mlir_opt_exe $<TARGET_FILE:mlir-opt>)
    set(mlir_opt_target "mlir-opt")
  else()
    set(mlir_opt_exe "${LLVM_TOOLS_BINARY_DIR}/mlir-opt${CMAKE_EXECUTABLE_SUFFIX}")
    set(mlir_opt_target "")
  endif()

  if (LLVM_NATIVE_TOOL_DIR AND EXISTS "${LLVM_NATIVE_TOOL_DIR}/mlir-translate${LLVM_HOST_EXECUTABLE_SUFFIX}")
    set(mlir_translate_exe "${LLVM_NATIVE_TOOL_DIR}/mlir-translate${LLVM_HOST_EXECUTABLE_SUFFIX}")
    set(mlir_translate_target "")
  elseif (TARGET "host_mlir-translate")
    get_native_tool_path(mlir-translate mlir_translate_exe)
    set(mlir_translate_target "host_mlir-translate")
  elseif (LLVM_USE_HOST_TOOLS)
    get_native_tool_path(mlir-translate mlir_translate_exe)
    set(mlir_translate_target "host_mlir-translate")
    build_native_tool(mlir-translate mlir_translate_exe)
    add_custom_target(host_mlir-translate DEPENDS ${mlir_translate_exe})
    get_subproject_title(subproject_title)
    set_target_properties(host_mlir-translate PROPERTIES FOLDER "${subproject_title}/Native")
  elseif (TARGET "mlir-translate")
    set(mlir_translate_exe $<TARGET_FILE:mlir-translate>)
    set(mlir_translate_target "mlir-translate")
  else()
    set(mlir_translate_exe "${LLVM_TOOLS_BINARY_DIR}/mlir-translate${CMAKE_EXECUTABLE_SUFFIX}")
    set(mlir_translate_target "")
  endif()
